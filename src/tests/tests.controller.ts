...
import { ApiTags, ApiOperation, ApiBody, ApiParam, ApiResponse, ApiConsumes, ApiBearerAuth } from '@nestjs/swagger';
import { TestSettingsDto, GenerateDto, SubmitAnswerDto, SubmitTestDto } from './dto/test.dto';
...
export class TestsController {
  ...
  async uploadTest(@UploadedFile() file: Express.Multer.File, @Req() req: Request, @CurrentUser() user: JwtUser) {
  ...
  async setSettings(@Param('testId') testId: string, @Body() body: TestSettingsDto, @CurrentUser() user: JwtUser) {
  ...
  @ApiResponse({ status: 404, description: 'Test not found' })
  async getSettings(@Param('testId') testId: string, @CurrentUser() user: JwtUser) {
  ...
  @ApiParam({ name: 'id', type: 'string', description: 'ID of the test' })
  @ApiBody({ schema: { type: 'object', properties: { variantId: { type: 'string' }, image: { type: 'string', format: 'binary' }, answers: { type: 'object', additionalProperties: { type: 'string' } } } } })
  @ApiConsumes('multipart/form-data', 'application/json')
  @UseInterceptors(PlatformFileInterceptor('image', {
    fileFilter: (req: Request, file: Express.Multer.File, cb: (error: Error | null, acceptFile: boolean) => void) => {
      if (file && !['image/png', 'image/jpeg'].includes(file.mimetype)) {
        return cb(new BadRequestException('Only PNG or JPEG images are allowed!'), false);
      }
      cb(null, true);
    },
    limits: { fileSize: 5 * 1024 * 1024 },
  }))
  @ApiResponse({ status: 201, description: 'Submission result', schema: { example: { score: 5, total: 10, breakdown: [{ question: 1, selected: 'B', correct: 'C', isCorrect: false }] } } })
  @ApiResponse({ status: 400, description: 'Invalid submission data or image format' })
  ...
  async submit(@Param('id') id: string, @UploadedFile() file: Express.Multer.File, @Body() body: any, @CurrentUser() user: JwtUser) {
    ...
    if (file) {
      ...
      const optionsPerQuestion = Math.max(...questions.map(q => q.options.length));
      
      const ocrResult = await ImageRecognitionService.analyzeCoverImage(file.buffer, {
      ...
      const breakdown = questions.map((q: any, qIdx: number) => {
        ...
        const isCorrect = !isMulti && !isUnanswered && selectedArr[0] === correctLabel;
      ...
      if (ocrResult.debugOverlayPath) response.debugOverlayPath = ocrResult.debugOverlayPath;
      if (ocrResult.warning) response.warning = ocrResult.warning;

      // Save submission
      await this.testsService.saveSubmission(id, variantId, user.userId, response);
  ...
  @UseGuards(RolesGuard)
  @Roles('admin')
  @ApiOperation({ summary: 'Export all submissions for a test as Excel (Admin only)' })
  @ApiParam({ name: 'id', type: 'string', description: 'ID of the test' })
  @ApiResponse({ status: 200, description: 'Excel file with all submissions', schema: { type: 'string', format: 'binary' } })
  @ApiResponse({ status: 404, description: 'Test not found' })
  async exportSubmissions(@Param('id') id: string, @Res() res: Response, @CurrentUser() user: JwtUser) {
